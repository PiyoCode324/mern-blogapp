...
const PostMenuActions = ({ post }) => {
  ...
  // デバッグログの追加
  console.log("--- DEBUG: PostMenuActions Component ---");
  console.log("Post received as prop:", post);
  if (post) {
    console.log("post.user:", post.user);
    if (post.user) {
      console.log("post.user.username:", post.user.username);
    } else {
      console.log("WARNING: post.user is undefined or null inside PostMenuActions.");
    }
  } else {
    console.log("WARNING: post prop is undefined or null in PostMenuActions.");
  }
  ...
  const isSaved = Array.isArray(savedPostIds) && savedPostIds.includes(post?._id); // postがundefinedの場合に備えてオプショナルチェイニングを追加
  ...
  // postがまだundefinedまたはnullの場合は何もレンダリングしない
  // これにより、post.userへのアクセスエラーを防ぐ
  if (!post) {
    return null; // あるいはローディングスピナーなどを表示
  }
  ...
  return (
    <div>
      ...
      {/* ここで post.user が undefined かどうかをチェックしてからアクセスする */}
      {user && post.user && (post.user.username === user.username || isAdmin) && (
        <div
          className="flex items-center gap-2 py-2 text-sm cursor-pointer"
